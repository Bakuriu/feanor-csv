...
def get_type(tree: ExprNode, env=None, func_env=None, compatible=default_compatibility):
    if env is None:
        env = {}
    if func_env is None:
        func_env = {}
...
def _get_type_from_definition(type_definition: TypeNameNode, env, func_env, compatible):
    return SimpleType(type_definition.children[0].name, type_definition.children[0].config)
...
def _get_type_from_assignment(assignment: AssignNode, env, func_env, compatible):
    ty = get_type(assignment.children[0], env, func_env)
...
def _get_type_from_projection(projection: ProjectionNode, env, func_env, compatible):
    ty = get_type(projection.children[0])
    ...
    return ParallelType(ty.types[index.literal] for index in projection.children[1:])


def _get_type_from_call(call: CallNode, env, func_env, compatible):
    arg_types = [get_type(arg, env, func_env) for arg in call.children[1:]]
...
def _get_type_from_merge(merge: BinaryOpNode, env, func_env, compatible):
    ...
    return MergeType([left_ty, right_ty], {'left_config': merge.children[2], 'right_config': merge.children[4]})
...
def _get_type_from_parallel_composition(parallel: BinaryOpNode, env, func_env, compatible):
    ...
    return ParallelType([left_ty, right_ty],
                        {'left_config': parallel.children[2], 'right_config': parallel.children[4]})
...
def _get_type_from_choice(choice: BinaryOpNode, env, func_env, compatible):
    ...
    return ChoiceType([left_ty, right_ty], {'left_config': choice.children[2], 'right_config': choice.children[4]})
