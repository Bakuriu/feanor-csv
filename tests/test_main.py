...
class TestMakeSchemaCmdline(unittest.TestCase):
    ...
    def test_can_make_schema_with_a_single_column(self):
        schema = make_schema_cmdline([('A', '#int')], [], show_header=True)
        ...
        expected_transformer = SimpleNamespace(name='transformer#0', inputs=['arbitrary#0'], outputs=['A'], transformer=IdentityTransformer(1))
        expected_transformer_copy = SimpleNamespace(name='transformer#1', inputs=['arbitrary#0'], outputs=['A'], transformer=IdentityTransformer(1))
    ...
    def test_can_make_schema_with_multiple_columns(self):
        schema = make_schema_cmdline([('A', '#int'), ('B', '#int')], [], show_header=True)
        self.assertTrue(schema.show_header)
        ...
        expected_transformer_B = SimpleNamespace(name='transformer#1', inputs=['arbitrary#1'], outputs=['B'], transformer=IdentityTransformer(1))
        ...
        expected_transformer_B_copy = SimpleNamespace(name='transformer#3', inputs=['arbitrary#1'], outputs=['B'], transformer=IdentityTransformer(1))
    ...
    def test_can_make_schema_with_transformers(self):
        ...
        expected_transformer_A = SimpleNamespace(name='transformer#1', inputs=['arbitrary#0'], outputs=['A'], transformer=IdentityTransformer(1))
        expected_transformer_B = SimpleNamespace(name='transformer#2', inputs=['arbitrary#1'], outputs=['B'], transformer=IdentityTransformer(1))
        expected_transformer_A_copy = SimpleNamespace(name='transformer#3', inputs=['arbitrary#0'], outputs=['A'], transformer=IdentityTransformer(1))
        expected_transformer_B_copy = SimpleNamespace(name='transformer#4', inputs=['arbitrary#1'], outputs=['B'], transformer=IdentityTransformer(1))
